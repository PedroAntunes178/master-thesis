...
The \acrfull{sbi} interface 

\begin{figure}[!h]
    \centering
    \includegraphics{riscv_sbi.pdf}
    \caption{Multiple stages of the Linux boot on \textit{RISC-V}.}
    \label{fig:riscv_sbi}
\end{figure}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=\linewidth]{linux_boot_flow.pdf}
    \caption{Multiple stages of the Linux boot on \textit{RISC-V}.}
    \label{fig:linux_boot_flow}
\end{figure}

The \textit{OpenSBI} software has little knowledge of the \acrshort{soc} specific hardware. Furthermore, the Linux kernel software is compiled so that it is platform-independent. Consequently, there needs to exist a way of passing the \acrshort{soc} hardware configuration to the software. For that purpose, developers have to create a \acrfull{dts} file and compile the \textit{OpenSBI} software so that it knows where the \acrshort{dts} is located in memory. The \acrlong{dts} is a file that describes the hardware in a \acrshort{soc} through nodes. \textbf{What is a device tree?}
