...
The \acrshort{plic} hardware has many features that the author will not discuss here. In this thesis project the author adapted a already developed \acrshort{plic} core to his work. To integrate an existing \acrshort{plic} core what is essential to understand is how the the interrupt notifications are handled by the \acrshort{cpu}, how the author can customize the \acrshort{plic} through the hardware module parameters and the memory mapped registers. 

Figure \ref{fig:plic_int_flow} was obtained from the \textit{RISC-V} \acrshort{plic} specification and shows a sketch of the interrupt flow between the \acrshort{plic} hardware and the \acrshort{cpu}. To receive an interrupt notification from the \acrshort{plic} unit the \acrshort{cpu} needs a external interrupt pin. ..
...
Developers set the hardware module parameters in their hardware description file when instantiating the \acrshort{plic} hardware. The \acrshort{plic} parameters defined can influence the number of resources the hardware utilises.

Table .. shows the memory mapped registers that the \textit{RISC-V} \acrshort{plic} specification defines. Most of the registers are grouped 
...
Lastly, it is essential to note that the \acrshort{plic} hardware only needs to generate the needed registers, depending on the passed parameters. Consequently, if the number of sources is 32, the \acrshort{plic} hardware should only create 32 Interrupt Source Priority registers, not 1023.
