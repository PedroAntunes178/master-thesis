...
The core id could have been read from the \acrshort{csr}, which saves its value. The \textit{RISC-V} instruction that does so is \enquote{csrr    \%0, mhartid}. An example of a C code integration would be code snippet \ref*{lst:read_mhartid}.
...
For each of the developed components the author developed a Makefile target that automatizes the build process. Furthermore a general target was created. The general target, called \enquote{build-OS}, calls all of the other targets and builds a full-featured minimal \acrshort{os}.

\subsection{Bootloaders}
Before running the Linux kernel software, the \acrshort{soc} has to run two Bootloaders firmware. The first firmware to execute is the stage 0 bootloader. The stage 0 bootloader is an adaptation of the bootloader firmware used in \textit{IOb-SoC}. The second bootloader is a \textit{RISC-V} specific software called \textit{OpenSBI}.
...
A device tree file is needed to execute the Linux kernel on the developed \acrshort{soc}. The device tree file describes the hardware components in a specific \acrshort{soc}. Each component of the \acrshort{soc} is represented as a node in the \acrfull{dts} file. The complete device tree can be seen in annex~\ref{section:annex1}.

The most important nodes are the \acrshort{cpu} and the memory. The \acrshort{cpu} node describes the \acrshort{cpu} hardware, supported architecture and specific features. The \acrshort{cpu} node description used by the author in the \acrfull{dts} was based on the description sugested by the \textit{VexRiscv} developer. The memory node indicates that the \acrshort{soc} \acrshort{dram} start address is 0x80000000 and it has 512MB of available memory (i.e. memory lenght is 0x10000000). The \acrshort{dts} atribute \enquote{regs} indicates the respective hardware devices addresses in the \acrshort{soc}. 

The \enquote{choosen} node is the only node that does not represent a real device. The \enquote{choosen} node is used to indicate the arguments that should be passed to the Linux kernel when it boots. The \enquote{choosen} node also indicates where in the memory the \acrlong{rootfs} is located.

The \acrshort{soc} peripherals are inside a node which the author called \enquote{soc}. The \acrshort{clint} node indicates the  \acrshort{clint} addresses and the interrupts it can create. The \acrshort{plic} is commented because althought it existed when included on the \acrshort{dts} it would cause the Linux kernel to panic and stop working. The \acrshort{uart} node indicated the systems baud rate, frequency, the \acrshort{uart} address and how the registers should be accessed. All the peripherals nodes have an atribute called \enquote{compatible} which indicates the device drives with which the component is compatible.
...
To build a Linux kernel compatible with the developed \acrshort{soc} the kernel had to be compatible with the \textit{VexRiscv} \acrshort{cpu} in it. The \textit{SpinalHDL} author has a Linux kernel fork compatible with the \textit{VexRiscv} \acrshort{cpu}. Consequently, the \textit{SpinalHDL} kernel fork is used by the user to create the kernel to run with the \acrshort{soc} developed in this project.

The thesis author developed a Makefile target, called \enquote{build-linux-kernel}, to automatically build the Linux kernel. The author also created a configuration file adapted to the \acrshort{soc}. The Makefile target has to copy the customized configuration file to the configs directory related to the \textit{RISC-V} \acrshort{isa}. Then the target has to configure the  kernel build process with the copied file. Finally, the kernel can be compiled and the resulting binnary image copied to the \acrshort{os} directory.
...
The author tested the build process using \textit{Buildroot} and was successful in creating a executable Linux kernel. However, the process of creating the Linux kernel with \textit{Buildroot} is slower than the previous method. For the development of the \acrshort{os}, \textit{Buildroot} could be useful in the future when creating a more complex system. A more complex system would be a \acrshort{soc} with more peripherals, for example an Ethernet peripheral.
