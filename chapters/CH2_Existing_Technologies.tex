...
RISC-V CPUs are most popular in simple systems. Consequently there exists a widely variety of open-source CPUs witch are implemented on multiple embedded microcontrollers. A few examples of those \acrshort{cpu}s would be the PicoRV32~\cite{picorv32}, NEORV32~\cite{neorv32}, DarkRISCV~\cite{darkriscv} and Ibex~\cite{ibex} from lowRISC. Those will not be discussed in detail on this paper since they do not meet the requirements to run the Linux Kernel. Those \acrshort{cpu}s generali support \acrfull{machine} level privilege mode and some also support \acrfull{supervisor} mode. Moreover, none of the given examples support the Atomic RISC-V \acrshort{isa} extension. This extension is essential to run Linux.

To run a Linux based Operating System it an application processor is needed. This means that the processor should have the required \acrfull{csr}, support \acrshort{machine}+\acrshort{supervisor}+\acrshort{user} privilege modes and support atomic instructions. An open-source solution would be either the \textit{CVA6}~\cite{zaruba2019cost} (previously known as Ariane), (\textit{BOOM}~\cite{zhaosonicboom}) or VexRiscv~\cite{vexriscv} from SpinalHDL.
